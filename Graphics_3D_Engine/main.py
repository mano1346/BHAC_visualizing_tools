...
class GraphicsEngine:
    def __init__(self, object_data, camPos=(15, 10, 15),show_screen=True,cb=None, win_size=(1600, 900)):
        # init pygame modules
        pg.init()
        # window size
        ...
        self.light = Light()
        # camera
        self.camera = Camera(self, position=camPos)
        # mesh
        self.mesh = Mesh(self)
        # scene
    ...
    def render(self):
        # clear framebuffer
        self.ctx.clear(color=(0.58, 0.58, 0.58))
        # self.ctx.clear(color=(0.08, 0.16, 0.18))S
        # render scene
        self.scene.render()
    ...
    def save_image(self,foldername, filename, format=".png"):
        ...
        if self.cb:
            image.paste(self.cb, (0,0), mask=self.cb)
